...
standard_cases = [s for s in scenarios if s.get("test_type", "").lower() in ["standard", ""]]
missing_body_cases = [s for s in scenarios if s.get("test_type", "").lower() == "missing_body"]
wrong_method_cases = [s for s in scenarios if s.get("test_type", "").lower() == "wrong_method"]
malformed_json_cases = [s for s in scenarios if s.get("test_type", "").lower() == "malformed_json"]
...
class TestBookSearchAPI:
    ...
    def test_search_books_standard(self, api_client, data):
