...
test.describe("Reset Password Page UI", () => {
  ...
  for (const data of testData) {
    test(`${data.test_case_id || data.id} | Reset Password test for email: ${data.email || "empty"}`, async ({
      ...
      if (["EMPTY_FIELD", "NULL_FIELD", "MISSING_EMAIL"].includes(email))
        email = "";
      if (email === "VALID_EMAIL") email = "john.doe@example.com";

      if (["EMPTY_FIELD", "NULL_FIELD", "MISSING_PASSWORD"].includes(password))
        password = "";
      ...
      const expectedMessage =
        data.expected_message || data.expected_result || "";

      // Success Validation
      if (
        expectedStatus === "200" ||
        expectedMessage.toLowerCase().includes("success")
      ) {
      ...
      // Failure Validation
        ...
        if (
          desc.includes("http method") ||
          desc.includes("request body") ||
          desc.includes("json") ||
          desc.includes("extra unexpected") ||
          desc.includes("sql") ||
          desc.includes("xss") ||
          desc.includes("data type")
        ) {
          return;
        }
        ...
        if (!email && email !== " ") {
          await expect(
            resetPasswordPage.getValidationMessage("Email is required"),
          ).toBeVisible();
        } else if (email && !email.includes("@")) {
          await expect(
            resetPasswordPage.getValidationMessage("Enter a valid email"),
          ).toBeVisible();
        } else if (!password) {
          await expect(
            resetPasswordPage.getValidationMessage("New password is required"),
          ).toBeVisible();
        } else if (password && password.length < 6) {
          await expect(
            resetPasswordPage.getValidationMessage(
              "Password must be at least 6 characters",
            ),
          ).toBeVisible();
        } else if (
          expectedStatus === "404" ||
          expectedMessage.toLowerCase().includes("not found") ||
          expectedMessage.toLowerCase().includes("could not find user")
        ) {
          const toast = resetPasswordPage.getToastMessage();
          await expect(toast).toBeVisible({ timeout: TOAST_TIMEOUT });
